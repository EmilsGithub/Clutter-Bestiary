...
import net.emilsg.clutter_bestiary.entity.client.animation.EmberTortoiseAnimations;
...
public class EmberTortoiseModel<T extends EmberTortoiseEntity> extends BestiaryModel<T> {
    ...
    public void setAngles(EmberTortoiseEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if (!entity.isShielding())
            this.animateMovement(EmberTortoiseAnimations.EMBER_TORTOISE_WALK, limbSwing, limbSwingAmount, 3f, 2f);
        if (entity.isShielding())
            this.updateAnimation(entity.shieldingAnimationState, EmberTortoiseAnimations.EMBER_TORTOISE_SHIELD, ageInTicks, 1f);
